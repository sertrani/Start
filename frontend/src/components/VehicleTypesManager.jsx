...
export default function VehicleTypesManager() {
  ...
  const add = async () => {
    ...
    try {
      await api.post("/vehicle-types", { name: name.trim(), weight: 1.0 });
      setName("");
  ...
  const saveEdit = async () => {
    try {
      await api.put(`/vehicle-types/${editing.id}`, { name: editing.name, weight: editing.weight ?? 1.0 });
      setEditing(null);
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4">
      ...
      <p className="text-sm text-slate-500">Definisci le tipologie di veicolo (es. Auto, Scooter, Altro). Sono usate nella scheda veicolo e nella strategia sospensioni.</p>
          ...
          <div key={t.id} className="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2" data-testid={`vtype-${t.id.slice(0, 8)}`}>
            {editing?.id === t.id ? (
              ...
              <>
                ...
                <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-500" onClick={() => setEditing({ ...t })} data-testid={`vtype-edit-${t.id.slice(0, 8)}`}><Pencil className="h-4 w-4" /></Button>
